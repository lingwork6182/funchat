...
# 加载模型配置信息
from src.app.config.server_config import HTTPX_DEFAULT_TIMEOUT, FSCHAT_MODEL_WORKERS
from src.app.config.model_config import LLM_MODELS, MODEL_PATH, ONLINE_LLM_MODEL
from src.app.config.basic_config import logger, log_verbose

# 获取Embedding 模型
from src.app.services.minx_chat_openai import MinxChatOpenAI
...
def fschat_openai_api_address() -> str:
    ...
    获取OpenAI的 接口访问地址
    """
    from src.app.config.server_config import FSCHAT_OPENAI_API
...
def fschat_controller_address() -> str:
    from src.app.config.server_config import FSCHAT_CONTROLLER
...
def get_model_worker_config(model_name: str = None) -> dict:
    ...
    from src.app.services import model_workers
...
def get_prompt_template(type: str, name: str) -> Optional[str]:
    ...
    from src.app.config import prompt_config
    import importlib
...
if __name__ == '__main__':
    ...
    print(ans)
    # {'host': '192.168.110.131', 'port': 20002, 'device': 'cuda', 'model_path_exists': True, 'model_path': '/home/00_rag/model/ZhipuAI/chatglm3-6b'}

    # 测试加载在线模型
    ans = get_model_worker_config("zhipu-api")
    print(ans)
    #{'host': '127.0.0.1', 'port': 21001, 'device': 'auto', 'api_key': '', 'version': 'glm-4', 'provider': 'ChatGLMWorker', 'online_api': True}
